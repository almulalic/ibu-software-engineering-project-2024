...
describe("Eventport application", () => {
	...
	it("Verify user can't sign up with invalid email address", async () => {
		...
		await SignupPage.signUpWithInvalidEmailAddress(
			firstName,
			lastName,
			displayName,
			email,
			errorMessageInvalidEmailAddress
		);
	});
	...
	it("Verify user can't sign up with password that is isn't at least 6 characters long", async () => {
		...
		await SignupPage.signUpWithShortPassword(
			firstName,
			lastName,
			displayName,
			email,
	...
	it("Verify user can't sign up without agreeing with TOC", async () => {
		...
		await SignupPage.signUpWithoutCheckingTOC(
			firstName,
			lastName,
			displayName,
			email,
			correctPassword,
			errorMessageAgreeWithTOC
		);
	});
